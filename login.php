...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	try{
    	...
    	if(!$user) throw new Exception("Correct Email and Password is required !");		/*if user array is true means means email or pass both are true.*/

    	$user_token = bin2hex(random_bytes(20));
    	$login_user_id = $user["id"];

    	$result_new = mysqli_query($con,"insert into tokens values (null,'$login_user_id','$user_token')");
